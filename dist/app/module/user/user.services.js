...
const createUser = (userData) => __awaiter(void 0, void 0, void 0, function* () {
    const { email, password, role } = userData, rest = __rest(userData, ["email", "password", "role"]);
    const isUserExits = yield user_model_1.User.findOne({ email });
    if (isUserExits) {
        throw new Error("user already exists");
    }
    const hashPassword = yield bcrypt_1.default.hash(password, 10);
    const user = yield user_model_1.User.create(Object.assign({ email, password: hashPassword, role }, rest));
    return user;
});
...
const promoteToAgent = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const user = yield user_model_1.User.findByIdAndUpdate(id, { role: "AGENT" }, { new: true, runValidators: true }).select("-password");
    return user;
});
